...
@Slf4j
@NoArgsConstructor
public class RIPEMD320StrategyImpl implements CryptoHashStrategy {

    private final RIPEMD320Digest ripemd320 = new RIPEMD320Digest();
    ...
    public EncryptionResponseDTO encrypt(String password) {
        ...
        var end = Instant.now();
        log.info(LOG001, "encrypt", "RIPEMD320", Duration.between(start, end).toMillis());
    ...
    public PasswordMatchingResponseDTO matches(PasswordMatchingRequestDTO passwordMatchingRequestDTO) {
        ...
        var end = Instant.now();
        log.info(LOG001, "match", "RIPEMD320", Duration.between(start, end).toMillis());
        return new PasswordMatchingResponseDTO(match);
    }

    private String encrypt(byte[] text) {
        ripemd320.update(text,0,text.length);
        var encrypted = new byte[ripemd320.getDigestSize()];
        ripemd320.doFinal(encrypted,0);
