...
@Slf4j
@NoArgsConstructor
public class RIPEMD256StrategyImpl implements CryptoHashStrategy {

    private final RIPEMD256Digest ripemd256 = new RIPEMD256Digest();
    ...
    public EncryptionResponseDTO encrypt(String password) {
        ...
        var end = Instant.now();
        log.info(LOG001, "encrypt", "RIPEMD256", Duration.between(start, end).toMillis());
    ...
    public PasswordMatchingResponseDTO matches(PasswordMatchingRequestDTO passwordMatchingRequestDTO) {
        ...
        var end = Instant.now();
        log.info(LOG001, "match", "RIPEMD256", Duration.between(start, end).toMillis());
        return new PasswordMatchingResponseDTO(match);
    }

    private String encrypt(byte[] text) {
        ripemd256.update(text,0,text.length);
        var encrypted = new byte[ripemd256.getDigestSize()];
        ripemd256.doFinal(encrypted,0);
