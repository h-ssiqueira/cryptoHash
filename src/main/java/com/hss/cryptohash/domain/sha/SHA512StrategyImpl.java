...
@Slf4j
@NoArgsConstructor
public class SHA512StrategyImpl implements CryptoHashStrategy {

    @Override
    public EncryptionResponseDTO encrypt(String password) {
        var start = Instant.now();
        var encrypted = sha512Hex(password);
        var end = Instant.now();
        log.info(LOG001, "encrypt", "SHA512", Duration.between(start, end).toMillis());
    ...
    public void matches(PasswordMatchingRequestDTO passwordMatchingRequestDTO) {
        var start = Instant.now();
        var password = sha512Hex(passwordMatchingRequestDTO.rawPassword());
        var match = new ByteComparator().compare(password.getBytes(), passwordMatchingRequestDTO.encryptedPassword().getBytes());
        var end = Instant.now();
        log.info(LOG001, "match", "SHA512", Duration.between(start, end).toMillis());
        if (match != 0) {
