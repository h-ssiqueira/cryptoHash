...
@Slf4j
@NoArgsConstructor
public class SHA1StrategyImpl implements CryptoHashStrategy {

    @Override
    public EncryptionResponseDTO encrypt(String password) {
        var start = Instant.now();
        var encrypted = sha1Hex(password);
        var end = Instant.now();
        log.info(LOG001, "encrypt", "SHA1", Duration.between(start, end).toMillis());
    ...
    public PasswordMatchingResponseDTO matches(PasswordMatchingRequestDTO passwordMatchingRequestDTO) {
        var start = Instant.now();
        var password = sha1Hex(passwordMatchingRequestDTO.rawPassword());
        var match = new ByteComparator().compare(password.getBytes(), passwordMatchingRequestDTO.encryptedPassword().getBytes());
        var end = Instant.now();
        log.info(LOG001, "match", "SHA1", Duration.between(start, end).toMillis());
