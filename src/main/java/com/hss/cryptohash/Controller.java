...
public class Controller {
    ...
    private final SCryptPasswordEncoder sCryptPasswordEncoder = new SCryptPasswordEncoder(65_536, 8, 1, 32, 16);
    private final Pbkdf2PasswordEncoder pbkdf2PasswordEncoder = new Pbkdf2PasswordEncoder("", 16, 310_000, PBKDF2WithHmacSHA256);
    ...
    // curl -X POST -H "Content-Type: text/plain" -d 'hash' http://localhost:8080/hash/password
    @POST
    @Path("/{password}")
    @Produces(TEXT_PLAIN)
    @Consumes(TEXT_PLAIN)
    public Response checkHashes(String hash, @PathParam(value = "password") String password) {
        return argon2PasswordEncoder.matches(password, hash) ? Response.ok("OK\n").build() : Response.status(400).entity("NOT OK\n").build();
//        return bCryptPasswordEncoder.matches(password, hash) ? Response.ok("OK\n").build() : Response.status(400).entity("NOT OK\n").build();
//        return sCryptPasswordEncoder.matches(password, hash) ? Response.ok("OK\n").build() : Response.status(400).entity("NOT OK\n").build();
//        return pbkdf2PasswordEncoder.matches(password, hash) ? Response.ok("OK\n").build() : Response.status(400).entity("NOT OK\n").build();
    }
